...
module.exports = data => {
  ...
  const runConfig = (serverOrUniversalPath) => {
    if (serverOrUniversalPath && serverOrUniversalPath !== 'default') {
    const serverSideRender = 
    require(`${path.join(dir, '..', 'template', serverOrUniversalPath)}`);
    const serverSideRenderConfig =
    fs.createWriteStream(`${path.join(dir, '..', 'server', 'server.js')}`, {
          flags: 'w+',
 ...
 let prodPath = (renderLogic === 'Server' || renderLogic === 'Universal') ? 
 'react-prod-template.js' : 'prod-template.js';
 // eslint-disable-next-line  no-irregular-whitespace
 let devPath = (renderLogic === 'Server' || renderLogic === 'Universal') ? 
 'react-dev-template.js' : 'dev-template.js';
 const prodTemplate = require(`${path.join(dir, '..', 'template', prodPath)}`);
 const prodConfig = 
 fs.createWriteStream(`${path.join(dir, '..', 'webpack', 'prod.config.js')}`, {
   flags: 'w+',
   defaultEncoding: 'utf8',
   mode: 0o666,
 });
 ...
 prodConfig.end();
 const devTemplate = 
 require(`${path.join(dir, '..', 'template', devPath)}`);
 const devConfig = 
 fs.createWriteStream(`${path.join(dir, '..', 'webpack', 'dev.config.js')}`, {
   flags: 'w+',
   defaultEncoding: 'utf8',
   mode: 0o666,
 });
 ...
 devConfig.end();
 const devServer = 
 require(`${path.join(dir, '..', 'template', 'dev-server.js')}`);
 const serverConfig = fs.createWriteStream(
   `${path.join(dir, '..', 'webpack', 'webpack-dev-server.js')}`, {
   flags: 'w+',
   defaultEncoding: 'utf8',
   mode: 0o666,
 });
 ...
 if (addons.includes('None')) {
   serverOrUniversalPath = 'default';
   console.log('\nAddons includes the option none.\n');
   console.log('We will only provide with webpack configuration');
 ...
 if (renderLogic === 'Client') {
   ...
   if (addons.includes('None') || addons.length === 0) {
